...
from .views import task_list, task_page, task_form, create_task_for_exist_task, change_task_status, change_comment

urlpatterns = [
    path('<str:lab>/all/', task_list, name='task_list_for_current_lab'),
    path('<str:lab>/all/<int:pk>/', task_page, name='task_page_url'),
    path('<str:lab>/all/<int:task_pk>/change_comment/<int:pk>/', change_comment, name='change_comment_url'),
